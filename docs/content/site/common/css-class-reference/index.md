...
All plugins plugins use the same standard and special classes.
...
- title.function.invoke (special)
  ...
  have a style configured. If not you can create your own CSS or live with the fallback mechanism.

## New scopes (classes)

All themes out there do not have CSS for these. Define your own CSS if you want it. If not the
standard fallback mechanism applies.

- template-variable.context

  The _Context_ -- single `.`, `$` or leading `.`, `$.` -- is a special thing in Go/Hugo templating.
  We use a dedicated class here to allow emphasis. Use it in your CSS to create a different visual
  appearance for _Context_.
...
_hugo-html_ uses the [Highlight.js][] builtin `xml` for highlighting HTML as subLanguage. So there
might be others. Please get these from the Highlight.js grammar documentation.

This applies to all grammars or plugins that include _hugo-html_.

[Highlight.js]: https://highlightjs.readthedocs.io/

[^1]: we consider that as a _build_in_. It's not controlling the templating.
