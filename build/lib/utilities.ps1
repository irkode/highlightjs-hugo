...
function Test-File {
  ...
  try {
    ...
    Write-Error "Path [ $testPath ] is not a file"
  } catch {
    Write-Error "Path [ $testPath ] does not exist or is not a file"
    throw "$_"
  }
}
    ...
    If ($TestPath) {
      If (-Not (Test-Path $testPath)) {
        if ($CreateIfMissing) {
          ...
          Write-Error "Path [ $testPath ] does not exist, is not a folder or create failed"
          throw "$_"
        }
      }
      $testPath = Resolve-Path -Path $testPath -ErrorAction Stop
