...
const H4H_GRAMMAR_METHOD_CHAIN_HELPER = {
   ...
   variants: [
      ...
      { begin: [/\./, /\w+/], beginScope: { 1: 'property', 2: 'property.method', }, },
      // method sub-mode - starting with a WORD
      { begin: [/\w+/], beginScope: { 1: 'property.method', }, },
   ],
   contains: [H4H_GRAMMAR_DOT_PROPERTY_CHAIN],
};

const H4H_GRAMMAR_FUNCTION_KEYWORDS = {
   $pattern: /\w+\.\w+/,
   'built_in': h4hbase.H4HBASE_FUNCTIONS,
};
const H4H_GRAMMAR_PIPE_FUNCTION_MODE = {
   // scope: 'PIPE_FUNCTION_MODE',
   begin: h4hbase.H4HBASE_FUNCTIONS_REGEX_GROUPED,
   keywords: H4H_GRAMMAR_FUNCTION_KEYWORDS,
   contains: [H4H_GRAMMAR_METHOD_CHAIN_HELPER]
};
...
const H4H_GRAMMAR_PIPE_BUILTIN_MODE = {
   ...
   variants: [
      { begin: /\w+(?=\.)/, contains: [H4H_GRAMMAR_METHOD_CHAIN_HELPER] },
      { match: /\w+/ }
   ],
...
const H4H_GRAMMAR_PIPE_VARIABLE_MODE = {
   ...
   variants: [
      { begin: [/\$\w+(?=\.)/], beginScope: { 1: 'template-variable', }, contains: [H4H_GRAMMAR_METHOD_CHAIN_HELPER] },
...
const H4H_GRAMMAR_PIPELINE = [
   ...
   H4H_GRAMMAR_PIPE_FUNCTION_MODE,
