...
- hugo - short name
- hugo-html - full name
- hugo.txt - used to emulate text mode

#### Highlighting for text templates
...
You can emulate text mode by resetting the `hljs` styles for the xml part. Something like this:
...
- We mark the following _Handlebars_ opening template tags as invalid for hugo: `{{#`, `{{>`, `{{!--`, `{{!`

  `IgnoreIllegals` default value is `false` since version 11. So this stops highlighting with the hugo module

Known limitation:

- no support for Hugo shortcodes at the moment
...
[HighlightJS 4 Hugo]: https://github.com/irkode/highlightjs-hugo
